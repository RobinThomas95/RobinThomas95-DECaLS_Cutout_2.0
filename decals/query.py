...
class decals_cutout(BaseQuery):
    ...
    def query_position(self, coordinates, size, download_type='both', galid=None,layer='ls-dr9-resid'):
        """
        ...
        if download_type in ('jpg', 'both'):
            self.download_image(galid, ra, dec, size,layer=layer)
        if download_type in ('fits', 'both'):
            self.download_fits(galid, ra, dec, size,layer=layer)

    def query_name(self, obj_name, size, download_type='both', galid=None,layer='ls-dr9-resid'):
    
        coords=SkyCoord.from_name(obj_name)     
        galid = galid or obj_name
        download_type=download_type
        layer=layer
        self.query_position(coords, size=size, download_type=download_type, galid=galid, layer=layer)
